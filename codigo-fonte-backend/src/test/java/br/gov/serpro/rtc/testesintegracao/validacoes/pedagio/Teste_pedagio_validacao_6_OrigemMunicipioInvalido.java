...
class Teste_pedagio_validacao_6_OrigemMunicipioInvalido {
    ...
    @BeforeEach
    void beforeEach(
            final @Value("classpath:entradas/validacoes/pedagio/Teste_pedagio_validacao_6_OrigemMunicipioInvalido.json") Resource resourceFile)
            throws Exception {
        operacao = objectMapper.readValue(resourceFile.getInputStream(), PedagioInput.class);
    }

    @Test
    void teste_service_OrigemMunicipioInvalido() {
        assertThatThrownBy(() -> pedagioService.calcularCIBS(operacao))
                .isExactlyInstanceOf(MunicipioNaoEncontradoException.class);
    }

    @Test
    void teste_controller_OrigemMunicipioInvalido() throws Exception {
