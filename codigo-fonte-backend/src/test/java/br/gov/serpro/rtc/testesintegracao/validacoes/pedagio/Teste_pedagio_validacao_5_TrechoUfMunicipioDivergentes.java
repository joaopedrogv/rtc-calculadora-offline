...
class Teste_pedagio_validacao_5_TrechoUfMunicipioDivergentes {
    ...
    @BeforeEach
    void beforeEach(
            final @Value("classpath:entradas/validacoes/pedagio/Teste_pedagio_validacao_5_TrechoUfMunicipioDivergentes.json") Resource resourceFile)
            throws Exception {
        operacao = objectMapper.readValue(resourceFile.getInputStream(), PedagioInput.class);
    }

    @Test
    void teste_service_TrechoUfMunicipioDivergentes() {
        assertThatThrownBy(() -> pedagioService.calcularCIBS(operacao))
                .isExactlyInstanceOf(MunicipioNaoPertencenteException.class);
    }

    @Test
    void teste_controller_TrechoUfMunicipioDivergentes() throws Exception {
