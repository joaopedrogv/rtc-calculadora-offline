...
class Teste_pedagio_validacao_7_TrechoMunicipioInvalido {
    ...
    @BeforeEach
    void beforeEach(
            final @Value("classpath:entradas/validacoes/pedagio/Teste_pedagio_validacao_7_TrechoMunicipioInvalido.json") Resource resourceFile)
            throws Exception {
        operacao = objectMapper.readValue(resourceFile.getInputStream(), PedagioInput.class);
    }

    @Test
    void teste_service_TrechoMunicipioInvalido() {
        assertThatThrownBy(() -> pedagioService.calcularCIBS(operacao))
                .isExactlyInstanceOf(MunicipioNaoEncontradoException.class);
    }

    @Test
    void teste_controller_TrechoMunicipioInvalido() throws Exception {
